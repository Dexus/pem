...
while read line; do
  ...
  LIBRARY="${parseLibVersion[0]}"
  VERSION="${parseLibVersion[1]}"

  OPENSSL_DIR="${OPENSSL_LOCAL_DIR}/${LIBRARY}_v${VERSION}"
  OPENSSL_BIN="${OPENSSL_DIR}/bin/openssl"
  if [ ! -f ${OPENSSL_BIN} ]; then
    echo "No BIN found for ${OPENSSL_BIN}"
    continue
  fi
  printTable '|' "Library|Version\n${LIBRARY}| ${VERSION}"
  ${OPENSSL_BIN} "$@" || true
done
