 ...
 * @param {Boolean} tomd5 if true build md5 hash of the generated modulus
 * @param {Function} callback Callback function with an error object and {modulus}
 */
function getModulus(certificate, password, tomd5, callback) {
    if (!callback && !md5  && typeof password === 'function') {
        callback = password;
        password = undefined;
        md5 = false;
    }
    else if (!callback && md5 && typeof md5 === 'function') {
        callback = md5;
        md5 = false;
    ...
    spawnWrapper(params, certificate, function(err, code, stdout) {
        ...
        if (match) {
            return callback(null, {
                modulus: tomd5 ? md5(match[1]) : match[1]
            });
        } else {
 ...
 */
function checkCertificate(certificate, callback){
  var params;
  if (!callback && typeof certificate === 'function') {
      callback = certificate;
      certificate = undefined;
 ...
 * @param {Function} callback Callback function with an error object and a boolean valid
 */
function checkPkcs12(certificate, passphrase, callback){
  ...
  var params = ['pkcs12', '-info', '-in', '--TMPFILE--', '-passin', 'pass:' + passphrase];
  spawnWrapper(params, certificate, function(err, code, stdout) {
      if (err) {
          return callback(err);
      }
      callback(null, /MAC verified OK/im.test(stdout.trim()));
  });
}
