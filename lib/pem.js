...
function createPrivateKey(keyBitsize, options, callback) {
  ...
  openssl.exec(params, '(RSA |ENCRYPTED |)PRIVATE KEY', function (sslErr, key) {
    ...
    if (!sslErr && key && openssl.get('Vendor') === 'OPENSSL' && openssl.get('VendorVersionMajor') >= 3 && !getFips()) {
      var traditional_params = ['pkey', '-traditional', '-in', '--TMPFILE--']
      ...
      if (options && options.cipher && (Number(helper.ciphers.indexOf(options.cipher)) !== -1) && options.password) {
        ...
        helper.createPasswordFile({
          ...
          password: options.password,
          passType: 'in'
        }, traditional_params, delTempPWFiles)
      ...
      openssl.exec(traditional_params, '(RSA |ENCRYPTED |)PRIVATE KEY', [key], function (traditional_sslErr, traditional_key) {

        helper.deleteTempFiles(delTempPWFiles, function (traditional_fsErr) {
          debug('createPrivateKey', {
            sslErr: traditional_sslErr,
            fsErr: traditional_fsErr,
            key: traditional_key,
            keyLength: traditional_key && traditional_key.length
          })
          key = traditional_key
          done(traditional_sslErr || traditional_fsErr)
        })
      })
    } else {
