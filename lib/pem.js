...
function convertKeyToTraditional(key, searchString, options, callback) {
  ...
  if (!callback) {
    return new Promise(function (resolve, reject) {
      convertKeyToTraditional(key, searchString, options, function (err, result) {
        if (err) {
          return reject(err)
        }
        resolve(result)
  ...
  if (key && searchString) {
    if (!getFips() || !options || !options.cipher || (Number(helper.ciphers.indexOf(options.cipher)) === -1)) {
      var params = ['pkey', '-traditional', '-in', '--TMPFILE--']
...
function createPrivateKey(keyBitsize, options, callback) {
  ...
  openssl.exec(params, '(RSA |ENCRYPTED |)PRIVATE KEY', function (sslErr, key) {
    ...
    helper.deleteTempFiles(delTempPWFiles, function (fsErr) {
      ...
      convertKeyToTraditional(key, '(RSA |ENCRYPTED |)PRIVATE KEY', options, function (convertErr, keyData) {
        if (!convertErr) {
          key = keyData.key
        }
...
function createEcparam(keyName, paramEnc, noOut, callback) {
  ...
  openssl.exec(params, searchString, function (error, ecparam) {
    ...
    if (noOut) {
      convertKeyToTraditional(ecparam, searchString, function (error, keyData) {
        if (error) {
          return callback(error)
        }
        return callback(null, {
      ...
      openssl.exec(params, searchString, [ecparam], function (error, eckey) {
        ...
        convertKeyToTraditional(eckey, searchString, function (error, keyData) {
          if (error) {
            return callback(error)
          }
          return callback(null, {
  ...
  openssl.execBinary(args, tmpfiles, function (sslErr, stdout) {
    function done(err) {
      ...
      if (!err) {
        ...
        if (keybundle.key) {
          // convert to RSA key
          return convertKeyToTraditional(keybundle.key, '(RSA |)PRIVATE KEY', function (err, key) {
            if (err) {
...
function checkCertificate(certificate, passphrase, callback) {
  ...
  if (certificate.match(/BEGIN(\sNEW)? CERTIFICATE REQUEST/)) {
    ...
    params = ['pkey', '-noout', '-check', '-in', '--TMPFILE--']
  } else {
