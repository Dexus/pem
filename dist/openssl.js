...
const settings = {};
const tempDir = process_1.env.PEMJS_TMPDIR || (0, os_tmpdir_1.default)();
const versionRegEx = new RegExp('^(OpenSSL|LibreSSL) (((\\d+).(\\d+)).(\\d+))([a-z]+)?');
if ("CI" in process_1.env && process_1.env.CI === 'true') {
    if ("LIBRARY" in process_1.env && "VERSION" in process_1.env && process_1.env.LIBRARY != "" && process_1.env.VERSION != "") {
        const filePathOpenSSL = `./openssl/${process_1.env.LIBRARY}_v${process_1.env.VERSION}/bin/openssl`;
        if (fs_1.default.existsSync(filePathOpenSSL)) {
            process_1.env.OPENSSL_BIN = filePathOpenSSL;
        }
    }
}
/**
...
function spawn(callback, params, binary) {
    var pathBin = get('pathOpenSSL') || process_1.env.OPENSSL_BIN || 'openssl';
